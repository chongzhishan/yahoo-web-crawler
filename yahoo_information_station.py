...
url = 'https://movies.yahoo.com.tw/article_index.html'
information_station= requests.get(url)
objSoup = bs4.BeautifulSoup(information_station.text, 'lxml')

photos = []
for item in objSoup.find('div','informpage_boxr'):
    if isinstance(item, NavigableString):
        continue
    if isinstance(item, Tag):
        photo = item.img['src']
        title = item.find('div','txt')
        print("標題:\n",title.text.lstrip())
...
# 先開啟檔案, 再儲存圖片
    ...
    for diskStorage in picture.iter_content(10240):
        pictFile.write(diskStorage)
    pictFile.close()
yourPath = r'C:\Users\User\Desktop\新爬蟲\電影新聞'
...
# 先開啟檔案, 再儲存圖片
    ...
    for diskStorage in picture.iter_content(10240):
        pictFile.write(diskStorage)
    pictFile.close()
yourPath = r'C:\Users\User\Desktop\新爬蟲\專題報導'
allFileList = os.listdir(yourPath)
for file in allFileList:
    jpg = '.jpg'
    new_file = file+jpg
...
# 先開啟檔案, 再儲存圖片
    ...
    for diskStorage in picture.iter_content(10240):
        pictFile.write(diskStorage)
    pictFile.close()
yourPath = r'C:\Users\User\Desktop\新爬蟲\影評'
allFileList = os.listdir(yourPath)
for file in allFileList:
    jpg = '.jpg'
    new_file = file+jpg
